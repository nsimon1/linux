...
static int vc_sm_vchi_videocore_io(void *arg)
{
				    ...
				    uint32_t num_connections)
{
	...
	instance->io_thread = kthread_create(&vc_sm_vchi_videocore_io,
					     (void *)instance, "SMIO");
...
int vc_sm_vchi_send_msg(struct sm_instance *handle,
			enum vc_sm_msg_type msg_id,
			void *msg, uint32_t msg_size,
			void *result, uint32_t result_size,
			u32 *cur_trans_id, uint8_t wait_reply)
{
	int status = 0;
...
int vc_sm_vchi_alloc(struct sm_instance *handle, struct vc_sm_alloc_t *msg,
		     struct vc_sm_alloc_result_t *result,
		     uint32_t *cur_trans_id)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_ALLOC,
...
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_FREE,
...
int vc_sm_vchi_lock(struct sm_instance *handle,
		    ...
		    uint32_t *cur_trans_id)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_LOCK,
				   msg, sizeof(*msg), result, sizeof(*result),
				   cur_trans_id, 1);
}
...
int vc_sm_vchi_unlock(struct sm_instance *handle,
		      ...
		      u32 *cur_trans_id, uint8_t wait_reply)
{
	return vc_sm_vchi_send_msg(handle, wait_reply ?
...
int vc_sm_vchi_resize(struct sm_instance *handle, struct vc_sm_resize_t *msg,
		      uint32_t *cur_trans_id)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_RESIZE,
				   msg, sizeof(*msg), 0, 0, cur_trans_id, 1);
}

int vc_sm_vchi_walk_alloc(struct sm_instance *handle)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_WALK_ALLOC,
				   0, 0, 0, 0, 0, 0);
}
...
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_ACTION_CLEAN,
...
int vc_sm_vchi_import(struct sm_instance *handle, struct vc_sm_import *msg,
		      ...
		      uint32_t *cur_trans_id)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_IMPORT,
				   msg, sizeof(*msg), result, sizeof(*result),
				   cur_trans_id, 1);
}
...
int vc_sm_vchi_client_version(struct sm_instance *handle,
			      ...
			      uint32_t *cur_trans_id)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_CLIENT_VERSION,
				   //msg, sizeof(*msg), result, sizeof(*result),
				   //cur_trans_id, 1);
...
int vc_sm_vchi_client_vc_mem_req_reply(struct sm_instance *handle,
				       ...
				       uint32_t *cur_trans_id)
{
	return vc_sm_vchi_send_msg(handle, VC_SM_MSG_TYPE_VC_MEM_REQUEST_REPLY,
				   msg, sizeof(*msg), 0, 0, cur_trans_id, 0);
}
