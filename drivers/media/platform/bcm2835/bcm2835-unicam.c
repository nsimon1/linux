 ...
 * the standard V4L2_PIX_FMT_ values, hence 0xfffffffxx.
 */
...
/*
 * formats already includes the Bayer format that the source will be listing,
 * so one of the all_bayer_* formats in each case can be ignored.
 */
#define MAX_POSSIBLE_PIX_FMTS \
		(ARRAY_SIZE(formats) + \
		ARRAY_SIZE(all_bayer_bggr) - 1 + \
		ARRAY_SIZE(all_bayer_rggb) - 1 + \
		ARRAY_SIZE(all_bayer_grbg) - 1 + \
		ARRAY_SIZE(all_bayer_gbrg) - 1)
				    ...
				    const struct unicam_fmt *fmt)
{
	...
	active_fmt = &unicam->active_fmts[unicam->num_active_fmt];
